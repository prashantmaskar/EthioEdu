...
function news_detail($post_id)
    ...
    $data['banners'] = $this->Init_models->getadvertisebanners();
     ...
     function add_news_comment($ne_id)
    {
...
function in_parent($in_parent,$post_id,$store_all_id) 
    ...
    { 
           $result = $this->Init_models->tree_by_parent_news($post_id,$in_parent); 
...
if($resultcheck == '0'){
$query = $this->db->query('UPDATE tbl_postresponse SET presponse_like = presponse_like+1 where presponse_type = "news" AND presponse_id = "'.$data_array['res_id'].'"' );
 $result=$query->result();
}
